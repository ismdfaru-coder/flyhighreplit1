...
const visualFlightSearchFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (passengers) {
      queryParts.push(`${passengers}`, passengers > 1 ? 'adults' : 'adult');
    }
    if (flightClass && flightClass.toLowerCase() !== 'economy') {
      queryParts.push(flightClass);
    }
    
    const flightQuery = queryParts.join(' ');
    ...
    const url = `https://www.google.com/travel/flights?${queryParams.toString()}`;
    ...
    if (cheapestPrice !== null) {
      const dummyFlight: Flight = {
        ...
        from: { time: 'N/A', code: origin.substring(0, 3).toUpperCase() },
        to: { time: 'N/A', code: destination.substring(0, 3).toUpperCase() },
        duration: 'N/A',
        stops: 0,
      };
